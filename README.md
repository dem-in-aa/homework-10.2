...
Кластер представлен следующей конфигурацией:
- 3 узла RabbitMQ.
- 3 сети docker, чтобы иметь возможность имитировать сетевой раздел.
- 1 Узел HAProxy для запроса баланса нагрузки и обеспечения "отказоустойчивости".
- 1 сеть по умолчанию, позволяющая потребителям и производителям подключаться к узлам через HAProxy.

Для анализа работы кластера можно провести нагрузочное тестирование, подразумевающее контролируемую среду, переходящую от низких нагрузок к высоким; либо стресс-тестирование с использованием скриптов, ботов и различных инструментов отказа в обслуживании для наблюдения за производительностью веб-сайта во время пиковых нагрузок.
